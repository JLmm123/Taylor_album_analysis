...
const albums = async function(req, res) {
  ...
  SELECT *
  FROM Albums
  ORDER BY STR_TO_DATE(release_date, '%Y-%m-%d') DESC
  `, (err, data) => {
    if (err || data.length === 0) {
      console.log(err);
      res.json({});
...
const album_songs = async function(req, res) {
  ...
  connection.query(`
  SELECT s.song_id AS song_id, s.title AS title, s.number AS number, s.duration AS duration, s.plays AS plays
  FROM Albums a
